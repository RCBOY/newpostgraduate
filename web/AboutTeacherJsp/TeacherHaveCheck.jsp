...
//                    String hqlgetstu="select new com.model.Student(s.sid,s.sname,s.ssex,s.stel,s.major) from Student s, Major m ,Assistant a where s.major=m.mid and s.assistant=a.aid and a.acc_num="+aname;
                    String hql= "select new com.model.Iterm(i.iid,i.iname,i.mark,i.type,h.comment,h.time,h.hid) from Harvest h, Iterm i where h.iterms=i.iid and i.type='智育分' and i.type!='辅德' and i.type!='辅体' and i.type!='辅智' and h.comment!='待审核' and h.student="+sid;
...
//                        String coment=it.getComment();
                ...
                <tr>
                    <td><%=i1%></td>
                    <td><a href="/AboutTeacherJsp/TeacherCheckDetailIterm.jsp?hid=<%=hid%>&&i1=<%=i1%>"><%=iname%></a> </td>
                    <%--<td><%=type%></td>--%>
                    <%--<td><%=time%></td>--%>
                    <%--<td><%=mark%></td>--%>
                    <%--<td><%=coment%></td>--%>
